...
export class InstagramService {
    ...
    public getPictures(): Observable<InstagramImage[]> {
        ...
        try {
            return this.http.request( `${CORS_HOST}/${IG_API}` , { method: 'GET', headers }).map((res) => res.json().items);
        } catch (err) {
            console.log(err);
            return err;
        }
    ...
    public getOembed(imgUrl: string): Promise<any> {
        let headers = new Headers({
        ...
        // return this.http.get(`https://api.instagram.com/oembed?url=${imgUrl}&omitscript=true`, {headers}).toPromise().then( (res) => {
        //     return res.json();
        // });
    }
